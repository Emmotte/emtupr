...
export default function Navigation() {
  ...
  return (
    ...
    >
      <div className={`text-xl font-bold tracking-tight ${theme === 'dark' ? 'font-casual text-neutral-100' : 'font-sans text-black italic'}`}>
        {style === '95' ? <DecryptedText text="Emmett Tupper" /> : "Emmett Tupper"}
      </div>
      ...
      <nav className={`hidden md:flex items-center gap-8 text-xs uppercase tracking-widest ${theme === 'dark' ? 'font-mono text-neutral-400' : 'font-sans font-bold text-[#404040]'}`}>
        {links.map((link) => (
          ...
          >
            {style === '95' ? <DecryptedText text={link.name} /> : link.name}
          </NavLink>
        ))}
      </nav>
